...
MD//WORKSは、最新ブラウザが動作するほぼすべての環境に対応しています。ただし、ブラウザの設定によっては、保存、印刷、全画面表示、暗号化Appの作成など一部機能が利用できない場合があります。保存挙動のOSとブラウザによる違いは[こちら](#save-behavior)をご覧ください。

* **端末要件:** 最新ブラウザが動作するPC、Mac、iPad、Androidタブレット、およびChromebook等。
