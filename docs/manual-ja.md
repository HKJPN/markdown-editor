...
表示設定では、アウトライン表示、不可視文字、行番号、カレント行ハイライト、行の折り返し、テーマ、全画面表示、Spell (EN)、Zenモードなどを切り替えできます。
...
本項では、Ollamaなどのローカル環境の代わりに、Metaなどが提供する「外部のクラウドAPI」をブラウザ上で利用するための応用手順を解説します。（※付録1-1、1-3の「接続先URL」を書き換える手法を活用します）
...
1. [dev.meta.ai](https://dev.meta.ai) で開発者登録を行い、APIキーを発行します。（※現在、新規登録で**$20分の無料クレジット**が付与されます）
