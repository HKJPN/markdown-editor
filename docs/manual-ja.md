...
| --- | --- | --- | --- |
| **Windows** | 最初の保存時に場所を指定すれば、以降は直接「上書き保存」が可能です。 | 上書きできないが、履歴管理番号付きのダウンロードが可能。 | — |
| **macOS** | 最初の保存時に場所を指定すれば、以降は直接「上書き保存」が可能です。 | 上書きできないが、履歴管理番号付きのダウンロードが可能。 | 同左 |
| **iOS / iPadOS** | 上書きできないが、[ファイル」アプリを介して**任意フォルダに**履歴管理番号付きの共有保存が可能 | 同左 | 同左 |
| **Chromebook** | 最初の保存時に場所を指定すれば、以降は直接「上書き保存」が可能です。 | — | — |
| **Linux** | 最初の保存時に場所を指定すれば、以降は直接「上書き保存」が可能です。 | 上書きできないが、履歴管理番号付きのダウンロードが可能。 | — |
