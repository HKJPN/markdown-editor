...
MD//WORKS はインストールは不要で、Editor のHTMLファイルをブラウザで開くとそのまま利用できます。[MD//WORKS Editorのサイト](<https://hkjpn.github.io/markdown-editor/>)か直接起動も可能です。前回の未保存データが残っている場合は、復元確認が表示されることがあります。復元する場合は確認画面で復元を選択してください。
