...
* **① メニューバー**  
  ファイル、編集、表示、書式、ヘルプ、及び最新バージョン通知の各メニューへアクセスします。ファイルの新規作成、保存、書き出し、表示切替、ヘルプ確認などを行います。書式メニューでは、見出し、太字・斜体、リスト、タスク、リンク、表、Markdown整形などの書式操作を行えます。
...
MD//WORKS Editorでは、ローカル保存容量と安全性を守るため、300KBを超える画像とSVG画像は挿入できない仕様です（詳しくは「3-3. 画像を挿入する」をご覧ください）。
