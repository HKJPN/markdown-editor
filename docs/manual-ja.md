...
ブラウザAIを活用するには、Google ChromeでGemini in Chromeを利用する方法が便利です。利用可能な環境では、以下の手順で使い始めることができます。
...
### 11-2. 活用事例
日常的な校正、要約、用語ゆれ確認、章構成の見直しには、軽量なモデル（Gemini Flash等）を使用する方が、短時間に提案が得られ、執筆に集中できます。通常の執筆時はFlashに固定しておき、「長文全体の構成レビュー」や「日本語と英語の細かなニュアンスの整合性チェック」など、高度な比較・検証が必要な場面でのみ上位モデル（Gemini Pro等）に切り替えるといった運用を推奨します。以下に具体的なプロンプトの例を挙げました。
...
ブラウザAIを利用する場合、表示中または共有した文書内容がAIの処理対象になる可能性があります。機密情報、個人情報、社外秘文書を扱う場合は、共有する範囲やブラウザ側の設定を確認してください。

Chrome Geminiに文書内容を共有したくない場合は、MD//WORKSのタブをGeminiと共有しない設定にするか、AIの使用を禁止できるFirefox等のブラウザがお勧めです。

日常的な校正、要約、用語ゆれ確認、章構成の見直しには、通常はGemini Flashで十分です。長文全体の精密な構成レビューや高度な推論が必要な場合のみ、Gemini Proなどの上位モデルの利用を検討してください。
...
| 観点 | Copilot in Edge![Image: Geminiに相談ボタン](<images/Bcopilot.png>) | Gemini in Chrome![Image: Geminiに相談ボタン](<images/Bgemni.png>)  |
| --- | --- | --- |
| エディタ連携の体感 | △ テキスト全体を読み取れない場合あり | ◎ MD//WORKS上では比較的スムーズに解析可能 |
| 設計傾向 | 安全性・社内ポリシー重視 | 柔軟性・ブラウザAI連携重視 |
| 画面テキストの認識 | 表示範囲や選択範囲を中心に認識する傾向 | MD//WORKSでの検証では、複雑なDOM構造でも文書全体を認識しやすい |
| Shadow DOM等への対応 | 認識できない場合あり | MD//WORKSでの検証では、Shadow DOMを含む画面構造でも比較的認識しやすい |
| セキュリティ・組織利用 | Microsoft 365や管理ポリシーと合わせやすい場合がある | Googleアカウント、Workspace設定、組織ポリシーに依存 |
| 開発者向けの解析 | △ 対象範囲を選択して使うと安定 | ◎ コード説明・構成レビューにも使いやすい |


### スムーズな連携を重視する方

MD//WORKSでの検証では、Gemini in Chromeは本エディタの画面構造に対してスムーズに動作し、文書全体の校正、要約、用語ゆれ確認、構成レビューなどに活用しやすい印象です。執筆支援機能を積極的に使いたい方におすすめです。

### セキュリティや社内規定を最重視する方

Copilot in Edgeは、組織利用やデータ保護ポリシーとの相性を重視する場合に選択肢となります。表示中の内容だけを対象とした執筆支援等に適しています。一方で、テキスト全体をうまく読み取れない場合があり、コピーペーストを行う場面が増えます。
