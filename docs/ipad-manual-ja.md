...
### 13-2. 外付けキーボード利用時に、編集画面で Home / End キーを押しても、文書の先頭や末尾に移動しません。
...
### 13-3. 暗号化されたファイルの長期的なアクセスと互換性について
