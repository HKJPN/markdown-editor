...
* **① メニューバー**  
  ファイル、編集、表示、書式、ヘルプ、及び最新バージョン通知の各メニューへアクセスします。ファイルの新規作成、保存（共有）、書き出し、表示切替、ヘルプ確認などを行います。書式メニューでは、見出し、太字・斜体、リスト、タスク、リンク、表、Markdown整形などの書式操作を行えます。
...
MD//WORKS Editorでは、Editorで入力した通常の改行はPreviewでもそのまま改行として表示されます。

- Enterで改行すると、Previewでも同じ位置で改行されます。
- 1行空けると、新しい段落として表示されます。
- 見出し、箇条書き、表、コードブロックなどはMarkdownの書式に従って表示されます。

一般的な文書作成ソフトに近い見た目で文章を確認できます。
...
#### 3-4-2. Web上のMD//WORKSからローカルファイルを開く場合
...
>フォルダからの画像の自動読み込みができなくても「3-3. 画像を挿入する」を参考に、Markdownへ直接埋め込む（ドラッグ＆ドロップする）ことができます。作成したMarkdownファイルの画像はどの環境でも安定してインライン表示できるので、互換性を重視する場合にはおすすめです。
...
文章の装飾は、Markdown記法を直接入力する方法、ツールバーを使う方法、書式メニューを使う方法があります。

ツールバーと書式メニューは、同じ基本的なMarkdown書式機能を利用するための異なる入口であり、好みの方法を選べます。書式メニューでは、見出し1 / 2 / 3、太字、斜体、取り消し線、上付き、下付き、インラインコード、引用、コードブロック、箇条書きリスト、番号付きリスト、タスク、リンク、表、水平線、Markdownを整形、目次を挿入・更新を利用できます。
...
見出しを作るには、ツールバーの **H1 / H2 / H3** ボタン、または **書式** メニューの対応する項目を使用します。
...
文字を選択してから、ツールバーのボタンを押すか、**書式** メニューの対応する項目を選ぶと、選択した文字に装飾を付けられます。
...
箇条書きやタスク管理用のチェックリストも作成できます。1行又は複数行を同時選択し、ツールバーのリスト、番号、タスクのボタンを押すか、**書式** メニューの対応する項目を選ぶことでも設定可能です。複数回押すことで再設定や解除が可能です。
...
3. 「”ファイル”に保存」をタップし、「ファイル」アプリから「このiPad内」や「iCloud Drive内」などの保存先を選択し、右上の「保存」をタップします。
5. Markdownファイルとして保存されます。

> **※注意：**
> ブラウザ（Safariなど）のメニューにある共有ボタン<img src="images/DoNotTapBrowserShareBottons.jpeg" width="20">ではなく、必ず**エディタのタイトルバー（画面内）にある共有ボタン<img src="./images/share-ja.jpg" alt="share" width="100">**を押してください。ブラウザの共有ボタンからはMarkdownファイルを保存できません。
...
複数タブで編集している場合、作業履歴は各タブのWorkspaceごとに独立して管理されます。**ブラウザを再起動した直後は、保存された全Workspaceの履歴がまとめて表示され、そこから復元できます。**各タブで編集を開始すると、そのタブの履歴のみが表示されます。保存後の履歴には、保存時に確定した実際のファイル名が表示されます。
...
MD//WORKS Editorでは、文書内の文字列を検索・置換する機能に加え、Markdownを整形し、目次を作成する機能等が搭載されています。検索・置換は **編集** メニュー、Markdownを整形と目次を挿入・更新は **書式** メニューから利用できます。長い原稿、議事録、仕様書、研究メモなどを編集する場合に便利です。
...
**書式 > Markdownを整形** を選択すると、Markdown文書内の余分な空行や行末スペースを整えることができます。
提出前、共有前、エクスポート前に使うと、文書の見た目や管理がしやすくなります。

### 8-5. 目次を挿入する

**書式 > 目次を挿入・更新** を選択すると、文書内の見出し（`#`〜`###` / H1〜H3）を元に、クリックして移動できるリンク付きの目次が自動作成されます。
...
* **目次を手動で編集する場合**
  挿入された目次は通常のテキストとして自由に書き換えられます。ただし、再度 **書式 > 目次を挿入・更新** を実行して目次を更新すると、**目次内で手動修正した内容は、最新の見出し構成に基づいてリセットされます**。手動で微調整する場合は、書き出し直前の「最後の最後」に行うのがおすすめです。
...
**v1.6.4以降:** Deep編集モードをONにすると、そのON/OFF設定がブラウザのローカルストレージに保存されます。ONのままページを閉じた場合、同じMD//WORKSを次回開くと、確認ダイアログを表示せずに新しいDeep編集セッションが自動的に開始されます。**DEEP** ボタンまたは **表示 > Deep編集モード** からOFFにすると設定も保存され、次回起動時もOFFになります。

永続化されるのはモードのON/OFF設定だけで、以前のDeep編集ログは復元されません。ページを再読み込みした後は、空の新規セッションとして記録が始まります。必要なログは、再読み込みする前にコピーしてください。
...
### 13-5. 外付けキーボード利用時に、編集画面で Home / End キーを押しても、文書の先頭や末尾に移動しません。
...
MD//WORKSは、[GitHub Flavored Markdown（GFM）](https://github.github.com/gfm/)を基本としています。見出し、太字、リスト、表、チェックリスト、コードブロックなどの一般的なGFM記法に加え、脚注、上付き・下付き文字、LaTeX数式など、科学・技術文書向けの記法を独自に拡張しています。そのため、厳密には純粋なGFMではなく、「GFMを基本としたMD//WORKS拡張Markdown」と位置づけられます。基本的なMarkdown記法は多くのMarkdownエディタと互換性がありますが、脚注、数式、上付き・下付き文字などの表示は、他のアプリ側の対応状況によって異なる場合があります。
...
重要な原稿は、Private Storageだけに頼らず、Markdownファイル（`.md`）として別途端末内に保存しておくことをおすすめします。
...
### 13-19. Mermaid図が描画されない

Mermaidレンダラーは、Mermaid図を初めて表示するときに外部CDNから読み込まれます。そのため、図の初回描画にはインターネット接続が必要です。

オフライン、ネットワークタイムアウト、組織のフィルタリング、CDNの遮断などによってレンダラーを取得できない場合、Previewには図の代わりに元のMermaidソースが表示されます。接続が復旧するとPreviewが再描画され、失敗したレンダラーの読み込みが再試行されます。

Mermaid構文が不正な場合は、修正できるように警告と元のソースが表示されます。また、Viewer系のエクスポート時に図を描画できなかった場合も、エクスポートされたViewerには図の代わりにエラー情報と元のソースが出力されます。

#### 制限事項

* 1文書で描画できるMermaid図は最大20個です。
* 各Mermaidコードブロックのソースは最大50,000文字です。
* 上限を超えたコードブロックは図として描画されず、警告と元のソースが表示されます。
* 大規模な図は、複数文書または複数の小さな図へ分割することを推奨します。
