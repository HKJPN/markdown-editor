...
Brave Leoは、サイドバーを開いた時点の文書構造を解析できるため、長文全体の校正、要約、表記ゆれのチェック、構成レビューと相性がよいです。長い文書を一度に全面的に書き換えさせるよりも、まずは指摘事項を一覧化させると、iPad上でも確認と修正を進めやすくなります。

たとえば、「矛盾点」「表記ゆれ」「重複説明」「曖昧表現」「章構成の改善案」など、観点を分けて依頼すると、修正すべき箇所を把握しやすくなります。重要な原稿では、AIの提案をそのまま採用せず、Preview画面で文脈や体裁を確認しながら反映してください。
...
私たちの検証において、iPadユーザーに最もシームレスで効果的な体験を提供したのがBraveブラウザの「Leo」です。サイドバーを開いた時点の文書構造（スナップショット）を読み込むため、iPad上での文書全体の校正、要約、表記ゆれ（一貫性）のチェックに最適です。更新した文書を再度AIに認識させる場合も、シンプルに新しくLeoを立ち上げるだけです。
