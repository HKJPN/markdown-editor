...
### 3-3. テキストやURLを直接ドロップする
画像だけでなく、WebサイトのテキストやURLもドラッグ＆ドロップで簡単に挿入できます。
> 💡 **リサーチと執筆を同時に行う**：Split Viewで画面の一方にブラウザ（Safariなど）を開き、検索したWebページのURLバーや、選択したテキストを指やペンで長押しします。そのままエディター画面へドラッグ＆ドロップするだけで、コピー＆ペーストの手間を省いて直感的に情報をスクラップできます。
