...
PWA（Progressive Web App）対応を予定しています。これにより、MD//WORKS はブラウザベースでありながら、ネイティブアプリに近い実行環境で動作可能となります。具体的には、ホーム画面へのインストール、キャッシュを活用した高速起動、ネットワーク非接続時のよりスムーズなオフライン動作などをサポートし、実行環境の柔軟性と可用性が向上します。
