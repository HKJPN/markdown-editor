...
* 目次（TOC）の自動生成機能を追加しました。
* 大量のテキストを貼り付ける際の安定性を改善しました。
* iPad Safariの全画面表示に合わせて、メニュー配置を調整しました。
* Markdownプレビューで上付き文字（`<sup>`）と下付き文字（`<sub>`）の表示に対応しました。
* 「Markdownによる科学論文執筆ガイド」と「iPadユーザーガイド」を追加しました。
