...
セキュリティ関係のユーザーから要望のあった**Private App Export** を追加しました。  
ノートを **暗号化された単一のHTMLファイル** としてエクスポートでき、暗号化・復号などの処理は **端末内のみ**で実行されます（サーバーへの送信は行いません）。

<details>
<summary><strong>セキュリティ詳細</strong></summary>

- AES‑256‑GCM による暗号化  
- PBKDF2 による鍵導出（310,000回）  
- 完全ローカル処理（サーバー不要 / アップロードなし）

</details>

**あなたの文章を、守って、どこへでも。— 1ファイルで完結。**

---

## 今後追加予定の機能

次のバージョンの MD//WORKS では、日常的な執筆やドキュメント内の移動をより快適にするため、検索・アウトライン・プレビューまわりの操作性を改善を予定しています。

### 改善予定

- 検索結果が自動的に表示位置へスクロールされるようになります。
- アウトラインから選択した見出しへ、より正確にジャンプできるようになります。

### 追加予定

- ダイアログ、メニュー、各種UI状態に対する Esc キー操作を一元管理します。
- Help メニューと内蔵ドキュメントモーダルを追加します。
- プレビュー画面で Ctrl+A / Cmd+A に対応します。
- プレビューからのリッチコピーに正式対応します。
