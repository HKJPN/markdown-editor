...
MD//WORKSは、インストールもクラウドも不要なエディタです。ブラウザさえあれば「安全に持ち運べるアプリ」のように文章を管理できます。もちろん、**すべてローカルで処理**されます。文書を1つのHTMLとして書き出す**新機能**を使えば、**閲覧制限**から共同執筆まで、誰とでも即座にドキュメントを共有・**保護**できます。さらに、高度なブラウザAI連携によりブラウザ標準のAIサイドバーの活用も可能です。「文章の校正」 や 「翻訳」 はもちろん文章 「全体の構成案」 等 をAIにコピペなしで指示し、秒速で返答が得られ本来の執筆に集中することができます。
...
今回のアップデートでは、エクスポート（書き出し）機能のワークフローを大幅に刷新し、セキュリティと柔軟性が格段に向上しました。

* **書き出し機能の強化と専用UIの新設:** 新しく専用のエクスポート画面を導入し、目的に合ったフォーマットを直感的かつスピーディーに選択できるようになりました。さらに、第三者による不正アクセスや無断コピーを防ぐ「制限付きViewer」や「パスワード付きViewer」が簡単に出力できるようになり、大切なドキュメントをより安全に保護・共有できます。
* **アップデート通知機能の追加:** メニューバーから最新バージョンの有無がひと目で確認できるようになりました。現在のバージョンと最新バージョンの比較や、公式のリリースページへのアクセスもスムーズに行えます。24時間に最大1回の確認通信を発生させますが、最小限の通信で、文書はいままで同様に全てローカルで処理されます。
* **検索・置換のバグ修正:** 正規表現を用いた置換機能において、`\n`（改行）や `\t`（タブ）といったエスケープシーケンスが正しく解釈されない重大な問題を修正し、より正確で快適なテキスト編集が可能になりました。
...
* **脚注機能 (Footnotes)**：論文の引用や参考文献の記述に便利な、脚注記法をサポートする予定です。
* **Pandoc互換の上付き・下付き文字**：`^上付き^` および `~下付き~` 記法をネイティブサポート。専用のツールバーボタンから、フォーマットの適用・解除・切り替えが直感的に行えるようになります。
* **プレビュー集中モード (Preview Focus Mode)**：右側のプレビューエリアを全画面に拡大し、レンダリングされたドキュメントの確認や閲覧に没入できるモードを導入予定です。
* **行番号の表示 (Line Numbers)**：コーディングや学術論文の査読をサポートするため、論理行ベースの番号表示、カレント行のハイライト、ガター（行番号エリア）からの行選択、コードモード時の折り返し無効化など、執筆を助ける機能を段階的に実装していきます。
* **数式のレンダリング (Math Equations)**：数学や工学分野で多用される、複雑な数式の表示に対応する予定です。
* **ディープエディットモード (Deep Edit Mode)**：思考のプロセスや推敲の履歴を詳細にトラッキングし、より深く文章と向き合える画期的な新機能を計画中です。
