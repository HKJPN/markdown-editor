...
* 🔒 **大切なドキュメントを保護**:新しく追加された「**制限付き**」 および「**パスワード保護**付き」のビューアー出力機能により、第三者による不正アクセスや無断コピーをしっかり防ぎます。
...
* **Preview Focus Mode**:右側Previewを画面全体へ拡大して閲覧できるモードを予定しています。
* **行番号**；簡易コーディングと論文レビューの双方を支援できるよう、論理行ベースの画面表示、カレント行ハイライト、ガタークリックによる行選択、およびコーディングモードでの折り返し無効化等を段階的に実装する予定です。
