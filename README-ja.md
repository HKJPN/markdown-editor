...
**インストール不要。設定不要。すぐ書ける。HTMLアプリですが、全てはローカル処理されます。他のツールと異なり、文書を安全に保持しつつ、どのような環境でも編集可能とする機能が搭載されています。更に、ChromeのGemini等と組み合わせることで「AI支援型のライティング環境」も可能としてます。コピペの手間なく、今開いているエディタの内容をAIに読み取らせて「文章のブラッシュアップ」「誤字のチェック」「表の要約」「コードのレビュー」や「関数の説明」などが可能です。もちろん、この機能のカットも可能**
