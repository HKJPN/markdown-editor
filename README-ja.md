...
**MD//WORKS**は、インストールもクラウドも不要なエディタです。ブラウザさえあれば、手元のパソコンだけで「安全に持ち運べるアプリ」のように文章を管理できます。もちろん、すべてローカルで処理されます。さらに、**高度なブラウザAI連携**により**AIサイドバー**も活用できます。**「文章の校正」や「翻訳」** をAIにコピペなしで指示し、秒速で完了可能です。特にBrave Leoと併用すれば、WindowsはもちろんiPad環境において**「複数タブの横断認識・情報集約」** （商品比較や論文の複数引用など）がシームレスに行えます。これはMS Word等でも不可能な機能です。
