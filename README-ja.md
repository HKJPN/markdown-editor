...
* **AIを活用したい場合:** ChromeやEdgeなどのブラウザAI（GeminiやCopilot）を使用して文章の校正などを行う場合、テキストは各プロバイダのポリシーに従って処理されます。機密情報を扱う際は、AIサイドパネルを閉じるか、読み取り許可をオフにしてください。
* **完全なプライバシーを確保したい場合:** **Firefox**や**Brave**など、ネイティブなAIアシスタントが組み込まれていないブラウザをご使用いただくことで、意図しない外部連携を完全にシャットアウトした安全なオフラインエディタとして機能します。
