...
**MD//WORKSは、インストールもクラウドも不要なエディタです。ブラウザさえあれば、手元のパソコンだけで「安全に持ち運べるアプリ」のように文章を管理できます。複雑なツール（ZIPやTiddlyWikiなど）と違って、どんな環境でもパスワードで暗号化したものを簡単に開いて編集が可能。もちろん、すべてローカルで処理されます。さらに、- **高度なブラウザAI連携:** ChromeのGeminiやBraveのLeoをサイドバーで活用可能で、「文章の校正」や「翻訳」をAIにそのまま指示できます。特にBrave Leoを使用すれば、WindowsやiPad環境において**MS Wordでも不可能な「複数タブの横断認識・情報集約」**（商品比較や論文の複数引用など）もシームレスに行えます。** ブラウザAIを執筆支援ツールとして積極活用する方法や、逆にブラウザAIを使用せず安全性重視で運用する方法について[取扱説明書の「11. AI支援執筆」](https://github.com/HKJPN/markdown-editor/blob/main/docs/manual-ja.md#11-ai%E6%94%AF%E6%8F%B4%E5%9F%B7%E7%AD%86--)にて解説しています。
