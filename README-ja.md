...
* **安全な「単一HTML」エクスポート**  
  ドキュメントの共有と保護をよりスマートに。新しいエクスポート機能を使えば、作成したドキュメントを一つのHTMLファイルにまとめることができます。閲覧制限をかけて安全に配布することも、完全に編集可能な状態のドラフトとして誰かと共有することも、これ一つで全て完結します。
...
* **アプリ起動用アイコン**: よりネイティブアプリに近い感覚で利用できるよう、専用のアプリ設定アイコンを追加予定です。
* **「アプリ化」に関する説明セクション**: ブラウザからスタンドアロンアプリとしてインストールして使用する方法について、マニュアルに専用の解説セクションを追加予定です。
