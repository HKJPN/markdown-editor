# 🚀 MD//WORKS v1.6.0.1 — AIアシスタント融合スタンドアロン Markdown エディタ![License: MIT](https://img.shields.io/badge/License-MIT-blue.svg)
...
---
## 🎉 v1.6.0.1 の新機能とアップデート

### 🔄 より柔軟な表示に：行の折り返し（Wrap / No Wrap）切り替え

長文データの視認性が劇的に向上しました！用途に合わせて最適なビューを選択できます。

* **ワンクリックで切り替え**：［表示］>［行を折り返す］から、テキストの折り返しを簡単に切り替えられるようになりました。
* **コードやログもスッキリ**：ソースコード、CSV、ログファイル、Base64などの「長い1行」も、折り返さずに横スクロールでスマートに確認できます。
* **検索時も迷子にならない**：折り返しオフ（No Wrap）の状態でも、検索結果が画面外にある場合はピタリと見える位置まで自動で横移動します。

### ⚡ パフォーマンス向上：「すべて置換」がよりパワフルで安全に

大規模なドキュメント編集も、より安心・快適に行えるよう内部処理を大きくブラッシュアップしました。

* **進捗が見えて安心**：大量のテキストを置換する際、処理状況がインジケーターで表示されるようになりました。意図しない重複クリックを未然に防ぎます。
* **長文ファイルでの安定性アップ**：置換後の画面更新プロセスを最適化し、文字数の多いドキュメントでの動作がさらに軽快・安定しました。
* **スマートな Undo（元に戻す）**：`Ctrl+Z` で置換をキャンセルした際、テキスト全体が全選択されてしまう煩わしい問題を解消。置換前の自然な状態へスムーズに復帰します。

### 🛡️ セキュリティアップデート

より安心してお使いいただけるように、外部ライブラリ（マークダウンプレビュー、HTMLサニタライズ、Wordインポート）を最新版又は安定板にアップデートしました。
