...
* **完全ローカル、なのにAI対応**  
  MD//WORKSはクラウドに依存しない完全なローカル環境で動作します。データは手元のデバイスに安全に保管したまま、ブラウザ標準のAIサイドバーと連携。面倒なコピペ作業なしで、シームレスにAIを活用できます。もちろん、ローカルLLMもサポートされます。
* **安全な「単一HTML」エクスポート**  
...
今回のアップデートでは、論文やレポート作成を強力に後押しする機能の追加と、執筆ワークフローをより快適にするUIの改善を行いました。

* 📖 **待望の「脚注」記法をサポート！** <br>
学術執筆に欠かせない脚注（`[^1]`）にネイティブ対応しました。思考の流れを止めることなく注釈や参考文献を記述でき、Preview画面や書き出したViewer上では、美しく相互リンクされた脚注一覧が自動生成されます。
* 👀 **プレビューが「3ステップ」でより直感的に** <br>
Previewボタンをクリックするだけで、「分割表示（リアルタイム確認）」→「Focus Preview（全画面で集中閲覧）」→「編集画面のみ」をシームレスに切り替えられるようになりました。執筆フェーズに合わせた最適な画面レイアウトを瞬時に呼び出せます。
* 🧪 **上付き・下付き文字がさらに書きやすく** <br>
化学式や数式で多用する上付き・下付き文字について、従来のHTMLタグ（`<sup>`, `<sub>`）に加え、Pandoc互換のシンプルでタイプしやすい構文（`^上付き^`, `~下付き~`）をサポートしました。
* ✍️ **「まっさらな状態」から、すぐに執筆を** <br>
アプリを起動した際のサンプルテキストを廃止し、開いた瞬間、すぐにあなたのアイデアを書き始められるようにしました。
* <img src="./images/icon2s.png" alt="Image:icon" width="30">  **アプリ起動用アイコン**: よりネイティブアプリに近い感覚で利用できるよう、専用のアプリ設定アイコンを追加し、アプリとしてのインストール法詳細をマニュアルに記載しました。
* 🛡️**プライバシー第一のAI** <br>
情報セキュリティを特に気にされるユーザー向けに、ローカルLLMの設定ガイドをマニュアルに追記しました。
