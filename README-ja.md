...
**インストール不要。設定不要。すぐ書ける。HTMLアプリですが、全てはローカル処理されます。他のツールと異なり、文書を安全に保持しつつ、どのような環境でも編集可能とする機能が搭載されています。更に、ChromeのGemini等と組み合わせることでコピペの手間なく、執筆中の内容から「文章のブラッシュアップ」「誤字のチェック」「表の要約」「コードのレビュー」や「関数の説明」なども可能です。もちろん、この機能のカットも可能**。AI支援執筆の活用方法については、[マニュアルの「11. AI支援執筆」をご覧ください](docs/manual-ja.md#11-ai支援執筆)。
