...
## 🎉 v1.5.9の新機能 **〜 行番号表示の対応と、印刷・PDF出力がさらに使いやすく進化！ 〜**

### 📝 執筆・レビューがもっとスムーズに！

* **行番号の表示に対応しました**
エディタ上で「論理行番号」の表示が可能に！Markdownのレビューや簡易コーディングはもちろん、論文・特許明細書などの厳密な確認作業において、**行単位での指示や校正が劇的にスムーズ**になります。（※ `View` メニューからいつでもON/OFF可能です）
* **「今どこ？」を防ぐ、カレント行ハイライト** ✨
現在編集している行を控えめに優しくハイライトします。長文の編集中でも迷子にならず、**目の前の執筆にしっかり集中**できます。

### 🖨️ 印刷・PDF保存がよりスマート＆確実に！

* **出力メニューを分かりやすいモーダルに整理**
「Preview」「Markdownソース」「行番号付きMarkdownソース」の中から、用途に合わせて迷わず選択し、印刷やPDF保存ができるようになりました。
* **プレビュー印刷がより美しく、安定**
ブラウザごとの出力のばらつきを抑えるため、印刷専用レイアウトでの出力方式を採用しました。いつでも安定したレイアウトで印刷可能です。
* **安心のナビゲーション機能**
HTMLソースの印刷時など、Preview印刷ではレイアウトが崩れやすいケースにおいて、より適切な「Markdownソース印刷」を促す警告を表示するようにしました。

### 🛠️ もっと快適に。こだわりの細部アップデート

* **行番号表示のズレを解消**：行高を整数pxでピタッと固定。PCでもiPadOSでも、行番号と本文の縦位置がズレることなく美しく揃います。
* **検索時のスクロールを改善**：Base64などの非常に長い行が含まれているファイルでも、検索でヒットした場所へスムーズかつ正確にスクロールするように調整しました。
  
---

## ✨ 今後のアップデート予定

* **行番号の表示 (Line Numbers)**：ガター（行番号エリア）からの行選択やコードモード時の折り返し無効化など、執筆を助ける機能を更に実装予定です。
